...
CorrectionTask::CorrectionTask(std::string inFile, std::string incalib, bool isList) :
	...
	fManager(),
	write_tree_(true) ,
	fVarManager(new DataTreeVarManager(inFile, isList))
{
	out_file_->cd();
...
//          config->AddCorrectionOnQnVector(rescale);
	...
	// u-vectors from MDC
	fManager.AddDetector("TracksMdc", DetectorType::TRACK, "Phi", "Ones", {ycm, pt}, {1, 2});
