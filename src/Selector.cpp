...
bool Selector::IsCorrectEvent() {
  ...
  //          return false;
  if (!fEvent->GetTrigger(fTrigger)->GetIsFired())
    return false;
  if (fEvent->GetVertexPositionComponent(2) > 0 ||
      fEvent->GetVertexPositionComponent(2) < -60)
    return false;
  Float_t Rx = fEvent->GetVertexPositionComponent(0),
          Ry = fEvent->GetVertexPositionComponent(1);
